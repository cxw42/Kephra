...
sub apply_settings {
	...
	apply_EOL_settings($ep);
	apply_LLI_settings($ep);
	apply_caret_line_settings($ep);
	apply_indention_guide_settings($ep);

	# Margins on left side
	Kephra::App::EditPanel::Margin::apply_settings($ep);

	#misc: scroll width, codepage, wordchars
	apply_autowrap_settings($ep);
